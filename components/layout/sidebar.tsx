...
export function Sidebar({ mobileOpen = false, onMobileClose }: SidebarProps) {
  ...
  const content = (
    <nav className="flex flex-col gap-1 p-4">
      <p className="mb-2 px-3 text-xs font-semibold uppercase tracking-wide text-[var(--color-muted)]">
        Administration
      </p>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          <button
            ...
            onClick={onMobileClose}
          />
          <aside className="relative z-10 flex h-full w-72 max-w-[85vw] flex-col bg-[var(--color-card)] shadow-xl">
